...
// Forward trigger ke WA owner? (1 = aktif)
const FORWARD_OWNER = String(process.env.FORWARD_OWNER || '') === '1'
      ...
      if (isSourceTrigger(text)) {
        ...
        if (FORWARD_OWNER) {
          try {
            ...
            log('➡️ Trigger juga diforward ke Owner WA.')
          } catch (e) {
            log('⚠️ Gagal forward ke Owner WA:', e?.message || e)
          }
        }
      }
